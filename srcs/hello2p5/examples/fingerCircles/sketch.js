...
function drawRightButtons() {
  ...
  setTextStyle();
  textSize(14);
  text("🎆 Firework", width - 200, height - 55);         // 텍스트 중앙 위치

  fill(0, 200, 0, 180);                                  // 초록색 (Good)
  rect(width - 130, height - 80, 100, 50, 10);           // Good 버튼
  setTextStyle();
  text("👍 Good", width - 80, height - 55);              // 텍스트 중앙 위치

  fill(128, 0, 255, 180);                                // 보라색 (Color)
  rect(width - 150, height - 460, 100, 50, 10);          // 컬러 버튼 (상단 위치)
  setTextStyle();
  text("🎨 Color", width - 100, height - 435);           // 텍스트 위치
  noStroke();                                            // 테두리 해제
}
